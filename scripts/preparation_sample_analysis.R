...
rowMeans(logFC_metabolomics_TUvsNG[which(rownames(logFC_metabolomics_TUvsNG) == "Putrescine"),])
...
rowMeans(logFC_proteomics_TUvsNG[which(rownames(logFC_proteomics_TUvsNG) == "LAMA5"),], na.rm =T)
...
metabolomics_DE_t <- as.data.frame(read_delim("data/metabolomics_DE_t_TUvsNG.txt", 
                                              ...
                                              trim_ws = TRUE))
metabolomics_DE_t$logFC<- metabolomics_DE_t$logFC*-1
metabolomics_DE_t$t<- metabolomics_DE_t$t*-1

write_csv(metabolomics_DE_t, "C:/Users/DustinSchilling/Documents/GitHub/prot_met_workflow/data/metabolomics_adjusted.csv")



# compare to before 
metab_old <- as.data.frame(read_delim("data/metabolomics_DE_t_TUvsNG.txt", 
                                              delim = "\t", escape_double = FALSE, 
                                              trim_ws = TRUE))
